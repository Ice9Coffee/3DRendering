...
Nearestphotons::~Nearestphotons() {
    delete[] dist2, dist2 = nullptr;
    delete[] photons, photons = nullptr;
}
...
void Photonmap::Store( Photon photon ) {
	...
	box_min = Vector3( fmin( box_min.x , photon.pos.x ) , fmin( box_min.y , photon.pos.y ) , fmin( box_min.z , photon.pos.z ) );
	box_max = Vector3( fmax( box_max.x , photon.pos.x ) , fmax( box_max.y , photon.pos.y ) , fmax( box_max.z , photon.pos.z ) );
}
...
Color Photonmap::GetIrradiance( Vector3 O , Vector3 N , double max_dist , int n ) {
	...
	for ( int i = 1 ; i <= np.found ; i++ ) {
		...
		if ( dot(N, dir) < 0 ) ret = ret + np.photons[i]->bright;
	}
	
	ret = ret * ( 8 / ( emit_photons * np.dist2[0] ) );
	return ret;
}
...
void Photonmap::LocatePhotons( Nearestphotons* np , int index ) {
	...
	double dist2 = distance2(photon->pos, np->pos);
